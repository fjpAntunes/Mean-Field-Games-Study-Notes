...
\input{analyticalPointOfView/equations.tex}
...
\begin{cases}
        {\dot {\bf \state}}_s = H_p({\bf \state}, D_x V({\bf \state},s)),\\
        {\bf \state}_{t_0} = \state.
\end{cases}
\end{equation}
...
\begin{equation}\label{reduced_mfg:density_transport_pde}
    \begin{cases}
        \rho_t(x,t) + \nabla(\partial_p H(x, \partial_x V (x,t)) \rho(x,t)) = 0
        \rho(x,t_0) = \rho_0(x)
\end{cases}
\end{equation}
